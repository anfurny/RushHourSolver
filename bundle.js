...
/***/ function(module, exports, __webpack_require__) {
	...
	__webpack_require__(/*! ./rushHour.js */ 191);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var genericSearcher = __webpack_require__(/*! ./genericSearch.js */ 192);
	//var _ = require('lodash');
	
	var thePuzzle = "2333 4" + "2556 4" + "11 6 4" + "7789  " + "  89AA" + " BBCC ";
	
	var somePuzzle = FA(thePuzzle.split("")).chunk(6);
	
	var add = function add(coord1, coord2) {
	    return [coord1[0] + coord2[0], coord1[1] + coord2[1]];
	};
	
	var winCondition = function winCondition(puzzleState) {
	    return isVehicle(getValueOfSpace(puzzleState, [5, 2])) === 1;
	};
	
	var enumerateValidMoves = function enumerateValidMoves(puzzle) {
	...
	var canGoForward = function canGoForward(puzzle, vehicle) {
	    // forward = right or down
	    if (vehicle.orientation === 'ver') {
	        if (isSpaceOpen(puzzle, [vehicle.leftX, vehicle.topY + vehicle.size])) return { direction: 'down', vehicle: vehicle };
	    } else {
	        if (isSpaceOpen(puzzle, [vehicle.leftX + vehicle.size, vehicle.topY])) return { direction: 'right', vehicle: vehicle };
	    }
	    return false;
	};
	
	var canGoBackward = function canGoBackward(puzzle, vehicle) {
	    // backward = left or up
	    if (vehicle.orientation === 'ver') {
	        if (isSpaceOpen(puzzle, [vehicle.leftX, vehicle.topY - 1])) return { direction: 'up', vehicle: vehicle };
	    } else {
	...
	var applyMove = function applyMove(puzzleOriginal, move) {
	    var newPuzzle = puzzleOriginal.slice().map(".slice()");
	    var directions = { 'up': [0, -1], 'down': [0, 1], 'left': [-1, 0], 'right': [1, 0] };
	    var size = move.vehicle.size,
	        leftX = move.vehicle.leftX,
	        topY = move.vehicle.topY;
	
	    var vehicleTop = [leftX, topY];
	    var vehicleBottom = [leftX, topY + size - 1];
	    var vehicleLeft = vehicleTop;
	    var vehicleRight = [leftX + size - 1, topY];
	
	    var replacers = { 'up': [vehicleTop, vehicleBottom], 'down': [vehicleBottom, vehicleTop], left: [vehicleLeft, vehicleRight], right: [vehicleRight, vehicleLeft] };
	...
	var isSpaceOpen = function isSpaceOpen(puzzle, coord) {
	    return getValueOfSpace(puzzle, coord) === ' ';
	};
	
	var getValueOfSpace = function getValueOfSpace(puzzle, coord) {
	    var _coord = _slicedToArray(coord, 2);
	
	    var x = _coord[0];
	    var y = _coord[1];
	
	    if (puzzle.hasOwnProperty(y) && puzzle[y].hasOwnProperty(x)) return puzzle[y][x];
	    return "X";
	};
	
	var setValueOfSpace = function setValueOfSpace(puzzle, coord, value) {
	...
	var getVehicles = function getVehicles(puzzleOriginal) {
	...
	var isVehicle = function isVehicle(char) {
	    return "123456789ABCDEFGHIJ".indexOf(char) + 1;
	};
	
	var toString = function toString(puzzle) {
	    return "\n" + puzzle.map(".join('')").join("\n");
	};
	
	var findVehicleAt = function findVehicleAt(puzzle, coord) {
...
/***/ function(module, exports) {
	...
	var genericSearcher = function genericSearcher(initialState, getStateNeighbors, applyMove, endCondition, stateToString) {
	 ...
	 * @returns {*}
	 */
	genericSearcher.prototype.findSolution = function (resolve) {
	    var maxExecutes = arguments.length <= 1 || arguments[1] === undefined ? false : arguments[1];
	
	    var self = this;
	    if (maxExecutes === 0) {
	        throw "Invalid maxExecutes value";
	    }
	    if (this.queue.length) throw "Job already running";
	    this.queue.push(function () {
	        return self.iterate(self.initialState, false, true);
	    });
	 ...
	 * @returns {*}
	 */
	genericSearcher.prototype.consumeQueue = function (done) {
	    var maxExecutes = arguments.length <= 1 || arguments[1] === undefined ? 1000 : arguments[1];
